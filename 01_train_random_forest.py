...
def validate_classifier(model, X_test, y_test, activity_names, X_train=None, y_train=None, show_plots=True):
    ...
    # Accuracy per class (use train to determine number of classes if available)
    accuracy_per_class = []
...
def main_random_forest():
    ...
    print(f"Training set: {X_train.shape[0]} samples (70%) - FOR TRAINING ONLY")  # type: ignore
    print(f"Test set: {X_test.shape[0]} samples (30%) - FOR TESTING ONLY, NEVER FOR TRAINING")  # type: ignore
    
    # Train Random Forest on 70% train set
    ...
    # Validate classifier on 30% test set (with training data for training accuracy)
    metrics = validate_classifier(model, X_test, y_test, activity_names, X_train=X_train, y_train=y_train, show_plots=True)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "random_forest":
        ...
        # Default: Run Random Forest classifier
        print("="*80)
        print("RUNNING RANDOM FOREST CLASSIFIER")
        print("="*80)
        main_random_forest()
